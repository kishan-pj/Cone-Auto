...
def draw_vertical_line(image, start_point, end_point, color, thickness=1):
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                if len(orange_cones) > 0 and len(green_cones) > 0:
                    largest_orange_cone = max(orange_cones, key=lambda x: (x[3] - x[1]) * (x[2] - x[0]))
                    largest_green_cone = max(green_cones, key=lambda x: (x[3] - x[1]) * (x[2] - x[0]))
                    draw_vertical_line(frame, (int((largest_orange_cone[1] + largest_orange_cone[3]) / 2), largest_orange_cone[2]), (int((largest_green_cone[1] + largest_green_cone[3]) / 2), largest_green_cone[2]), (0, 0, 255))  # Red color
