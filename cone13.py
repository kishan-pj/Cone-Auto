    ...
    with tf.Session(graph=detection_graph) as sess:
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                # Calculate the FPS based on the time taken for the current frame and the previous frame
                fps = 1.0 / ((toc - prev_frame_time) if prev_frame_time else 1.0)

                # Store the current frame time for the next iteration
                prev_frame_time = toc

                # Display the FPS on the frame
                cv2.putText(frame, f"FPS: {fps:.1f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
